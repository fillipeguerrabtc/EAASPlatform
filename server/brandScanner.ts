/**
 * BRAND SCANNER 2.0 - SECURE VERSION
 * 
 * Extracts comprehensive brand identity from websites using:
 * - K-Means color clustering (CIELAB-based)
 ...
 * - Media blocking for performance
 * - Timeout enforcement
 * - Automatic cleanup
 */

import type { ThemeTokens } from '@shared/schema';
import { safeBrowserRequest } from './browser';
 ...
 * Uses secure browser automation with queue management and resource blocking
 */
export async function scanWebsiteBrand(url: string): Promise<BrandAnalysis> {
  return await safeBrowserRequest(
    async (page) => {
      ...
      console.log('✅ Brand Scanner 2.0 (SECURE) - Extraction Complete:', {
        kMeansClusters: 5,
    ...
    },
    {
      ...
      timeout: 30000,
      blockMedia: false, // We need to see images for logo extraction
    }
  );
}
