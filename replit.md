...
- **Marketing Automation Module (Production-Ready):** Comprehensive multi-channel marketing campaign system with LGPD/GDPR compliance:
  ...
  - **Security Hardening (Architect Approved):** Complete tenant isolation on ALL operations (reads + writes), isAuthenticated middleware on admin routes, public tracking endpoints validate trackingId with tenant lookup via sendId, crypto-secure tracking IDs (no Math.random()), Drizzle ORM upserts (no SQL injection), foreign keys CASCADE (template→campaign→send→event), graceful provider degradation (SMTP/Twilio optional), Redis error handling (queue disabled if unavailable), structured error logging (no silent failures)
- **AI Multimodal 2.0 System (Production-Ready - Architect Approved):** Autonomous AI knowledge base with multimodal ingestion (text/image/URL), ONNX embeddings, HNSW vector search, and hybrid RAG scoring:
  - **Database Tables (16):** `ai_documents` (ingested content), `ai_chunks` (text/image chunks), `ai_embeddings_text` (ONNX text vectors), `ai_embeddings_image` (ONNX image vectors), `ai_entities` (NER entities), `ai_chunk_entities` (entity links), `ai_entity_links` (knowledge graph), `ai_ann_meta` (HNSW index metadata), `ai_feedback` (user feedback for scoring), `ai_traces` (AI interaction logs), `ai_metrics` (aggregated metrics), `ai_eval_daily` (daily evaluations), `ai_governance` (policies), `ai_learning_log` (learning history), `ai_policies` (governance rules), `ai_interactions` (user interactions)
  - **Multimodal Parser (parser.full.ts):** Full document parsing with auto-detection: HTML (JSDOM), PDF (pdf-parse), DOCX (mammoth), PPTX (unzipper + XML), CSV (csv-parse), Images (Tesseract OCR + Jimp). URL fetching with SSRF protection (HTTPS + domain allowlist + timeout + size limit). Security: HARD DENY without allowlist prevents DNS rebinding/IPv4-mapped IPv6/metadata service attacks.
  - **ONNX Embeddings:** Text embeddings (MiniLM L6 v2, 384 dims) and image embeddings (MobileNetV2, 1280 dims) via ONNX Runtime. Fixed token_type_ids requirement for MiniLM. Models: `models/minilm.onnx` (87MB), `models/mobilenet.onnx` (14MB), `models/vocab.txt`, `models/tokenizer.json`.
  - **HNSW Vector Search:** Approximate Nearest Neighbor (ANN) search using HNSW (hnswlib-node). Separate indexes per tenant/modality (text.cosine.384, image.cosine.1280). Persistent binary storage in `data/ann/`.
  - **Hybrid RAG Scoring:** 5-weight system (α/β/γ/δ/ζ) combining vector similarity (α), graph PageRank (β), temporal decay (γ), user feedback (δ), and MMR diversity penalty (ζ). DEFAULT_WEIGHTS: α=0.6, β=0.15, γ=0.1, δ=0.1, ζ=0.05. Embedding cache enables diversity scoring without performance regression.
  - **REST API (7 endpoints):** `POST /api/ai/query` (multimodal RAG query with hybrid scoring), `POST /api/ai/ingest/text` (raw text ingestion), `POST /api/ai/ingest/file` (file upload with multer), `POST /api/ai/ingest/url` (URL ingestion with SSRF protection), `GET /api/ai/documents` (list documents), `GET /api/ai/documents/:id` (document details), `GET /api/ai/knowledge/entities` (knowledge graph entities)
  - **Knowledge Graph (kg.ts):** Entity extraction (NER), entity linking, PageRank-like centrality scoring. Knowledge graph stored in `ai_entities`, `ai_chunk_entities`, `ai_entity_links` tables.
  - **Security Hardening (Architect Approved):** SSRF protection via HARD DENY (URL ingestion disabled without ALLOWED_URL_DOMAINS env var), HTTPS-only, domain allowlist (exact + subdomain matching), 30s timeout, 50MB size limit, tenant isolation on all operations, isAuthenticated middleware, Zod validation on all inputs.
  - **Technologies:** ONNX Runtime (embeddings), hnswlib-node (HNSW vector search), Jimp (image decoding), Tesseract.js (OCR), pdf-parse (PDF), mammoth (DOCX), unzipper (PPTX), csv-parse (CSV), JSDOM (HTML), Drizzle ORM + Zod validation
  - **Deployment Configuration:** Set `ALLOWED_URL_DOMAINS="example.com,docs.google.com,github.com"` (comma-separated) to enable URL ingestion. Optional: `AI_OCR_LANG="eng+por"` for multilingual OCR. URL ingestion is DISABLED by default for security (prevents SSRF attacks).
  - **Autonomy Verification:** Smoke test (`server/ai/test-autonomy.ts`) proves 100% functionality: Parser (HTML/PDF/DOCX/PPTX/CSV/OCR) ✅, Text ingestion ✅, ONNX embeddings ✅, HNSW vector search ✅, Hybrid scoring (α/β/γ/δ/ζ) ✅. All tests PASSED.
