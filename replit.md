...
- **Brand Theming System (October 2025):** Complete global theme application system that dynamically applies tenant brand identity across admin dashboard and customer marketplace:
  - **BrandThemeProvider (client/src/components/brand-theme-provider.tsx):** Global React provider that loads tenant customTheme and applies colors system-wide via dynamic CSS variables. Converts HEX to HSL format (Tailwind-compatible). Creates dynamic `<style id="brand-theme-override">` tag that applies:
    - **Primary, Secondary, Accent colors:** Applied to both `:root` and `.dark` (works in light and dark modes)
    - **Background, Foreground colors:** Applied to `:root:not(.dark)` only (light mode only to preserve dark mode contrast)
    - **Related CSS vars:** --sidebar-primary, --ring, --chart-1, --chart-2, --card, --primary-foreground, --accent-foreground
  - **Auto-cleanup:** Always resets to EAAS defaults first (removes style tag, resets favicon to /favicon.svg, resets title), then applies tenant overrides if available. Prevents branding persistence after logout.
  - **TenantLogo component:** Displays tenant logo in header with fallback to EAAS logo. Shows tenant name.
  - **Tenant Settings UI:** Brand Scanner interface with website URL input (data-testid="input-website-url"), scan button (data-testid="button-scan-website"), color previews, and "Restaurar Padrão" button (data-testid="button-restore-defaults") that sends customTheme: null to backend.
  - **Integration:** BrandThemeProvider wraps entire App.tsx, TenantLogo in header. Theme applies globally across all routes (/admin/*, /shop, /my-account).
